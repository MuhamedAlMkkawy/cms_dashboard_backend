"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ComponentsService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const components_entities_1 = require("./entities/components.entities");
const typeorm_2 = require("typeorm");
const lodash_1 = require("lodash");
const mongodb_1 = require("mongodb");
const nestjs_i18n_1 = require("nestjs-i18n");
let ComponentsService = class ComponentsService {
    componentsRepo;
    i18n;
    constructor(componentsRepo, i18n) {
        this.componentsRepo = componentsRepo;
        this.i18n = i18n;
    }
    async getAllComponents() {
        const data = await this.componentsRepo.find();
        if (!data.length) {
            throw new common_1.NotFoundException(await this.i18n.translate('components.componentsService.NO_COMPONENTS_FOUND'));
        }
        return data;
    }
    async getSingleComponent(id) {
        const component = await this.componentsRepo.findOneBy({
            _id: new mongodb_1.ObjectId(id),
        });
        if (!component) {
            throw new common_1.NotFoundException(await this.i18n.translate('components.componentsService.COMPONENT_NOT_FOUND'));
        }
        return component;
    }
    async createComponent(data) {
        const components = await this.getAllComponents();
        const isComponentFound = components.find((item) => item.type == data.type);
        if (isComponentFound) {
            throw new common_1.BadRequestException(await this.i18n.translate('components.componentsService.COMPONENT_TYPE_EXISTS'));
        }
        const newComponent = this.componentsRepo.create(data);
        return await this.componentsRepo.save(newComponent);
    }
    async updateComponent(id, data) {
        const component = await this.getSingleComponent(id);
        const updatedComponent = (0, lodash_1.merge)(component, data);
        const updated = await this.componentsRepo.save(updatedComponent);
        return updated;
    }
    async deleteComponent(id) {
        const component = await this.getSingleComponent(id);
        await this.componentsRepo.remove(component);
        return {
            message: await this.i18n.translate('components.componentsService.COMPONENT_DELETED_SUCCESS'),
            data: null,
        };
    }
};
exports.ComponentsService = ComponentsService;
exports.ComponentsService = ComponentsService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(components_entities_1.Components)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        nestjs_i18n_1.I18nService])
], ComponentsService);
//# sourceMappingURL=components.service.js.map